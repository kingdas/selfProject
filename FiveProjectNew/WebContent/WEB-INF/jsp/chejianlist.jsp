...
<tr>
     ...
     style="font-size:18px; margin-left:60px;"><span >增加</span></button> 
     
     
     <button type="button" class="glyphicon glyphicon-pencil"
      aria-label="Left Align" id="show" 
      style="font-size:18px; margin-left:60px;"><span >修改</span></button>
...
<br/>
		<table class="table table-bordered" align="center">
			...
			<c:forEach items="${us}" var="u"  varStatus="i">
				...
				<td><c:forEach  items="${ti}" var="ti" begin="${i.count-1 }" 
					...
					<td class="sc2" ><a
						...
						<td class="sc1" ><a href="#" onclick=
						"Del(${u.shopId},'${u.shopAdmin}')">删除</a></td>
						<!-- <td class="sc3" >导出</td> -->
					</tr>
					</c:forEach>
			...
			action="${pageContext.request.contextPath}/user/find">   当前第${currentPage}页/共${x }页
            <a href="${pageContext.request.contextPath}/user/find?n=1">首页</a>
            <c:if test="${x>1}">
			<a href="${pageContext.request.contextPath}/user/find?n=${currentPage-1}"> 上一页</a>
          </c:if>
           <c:if test="${x==1}">
           <a>上一页</a>
     </c:if>
   <%-- <c:forEach items="${li}" var="li"> 
          ...
          <c:if test="${x==1}">
			<a href="${pageContext.request.contextPath}/user/find?n=${currentPage+1}">
				第二页 </a></c:if> <!--  当条件查询时只显示一页，点击第二页跳转到列表分页第二页 -->
				<c:if test="${x>1}">
				<a href="${pageContext.request.contextPath}/user/find?n=${currentPage+1}">
				下一页 </a></c:if>  <!-- 有多页时，下一页链接 -->
				<c:if test="${x>1}">
				 <a href="${pageContext.request.contextPath}/user/find?n=${x}">尾页</a>
				</c:if>   <!-- 多页时，点击跳转到尾页 -->
				<c:if test="${x==1}"> <!--  只有一页时，点击不起任何作用 -->
                 <a>尾页</a>
                 </c:if>
